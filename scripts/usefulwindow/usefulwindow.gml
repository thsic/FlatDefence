///@param spritepath,x,y,width,height,scale,alpha
//ウディタの万能ウィンドウっぽいやつ
//-------------注意-------------
//3の倍数のスプライトでないとバグるかも 
//drawイベントの中に置きましょう
...
var window_x = argument1
var window_y = argument2
var window_width = argument3
var window_height = argument4
var sprite_scale = argument5
var sprite_alpha = argument6

if(!surface_exists(usefulwindow_surface))//surfaceが作られていないなら作る
{
	//仮のsurface作成(ここから抜き取ってウィンドウを描画する)
	temp_surface = surface_create(sprite_get_width(sprite_path)*sprite_scale,sprite_get_height(sprite_path)*sprite_scale)
	o_gameMgr.usefulwindow_surface = surface_create(window_width,window_height)//ウィンドウを描画するsurface
	...
	surface_set_target(usefulwindow_surface)//ウィンドウの方セット
	//順番に左上、右上、左下、右下
...
{
	draw_surface(o_gameMgr.usefulwindow_surface,window_x,window_y)
